...
class FlowMatching(Base):
    ...
    def _prepare_t(self, steps=None, t_start=0.0):
        steps = self.inference_steps if steps is None else steps
        # SDEdit warm-start: spread the steps over the remaining range [t_start, 1]
        t_seq = torch.linspace(t_start, 1, steps + 1)
    ...
    def generate_iter(
        ...
        **kwargs_conditionals,
        ...
        # Check for warm-start arguments
        x_start = kwargs_conditionals.pop("x_start", None)
        t_start = kwargs_conditionals.pop("t_start", 0.0)
        ...
        # Full step count is kept even when starting late (spread over [t_start, 1])
        t_seq = self._prepare_t(t_start=t_start).to(x_device)
