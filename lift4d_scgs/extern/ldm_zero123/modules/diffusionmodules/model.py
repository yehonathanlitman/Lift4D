...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # compute attention: single head with head_dim = c
        b, c, h, w = q.shape
        q = q.reshape(b, 1, c, h * w).transpose(-2, -1)  # b,1,hw,c
        k = k.reshape(b, 1, c, h * w).transpose(-2, -1)
        v = v.reshape(b, 1, c, h * w).transpose(-2, -1)
