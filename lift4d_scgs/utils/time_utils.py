...
class DeltaNodeWarp(nn.Module):
    """
    Per-frame delta deformation using direct optimization of control point deltas.

    Same skinning approach as ControlNodeWarp (FPS control nodes + RBF KNN weights),
    but replaces the MLP with a directly optimizable (num_frames, num_nodes, 3) tensor.
    Each frame gets its own independent deformation — no temporal smoothing bottleneck.
    """

    def __init__(self, is_blender=True, node_num=512, K=3, num_frames=1,
                 with_arap_loss=True, d_rot_as_res=True, opt_deform_rot=False, **kwargs):
        super().__init__()
        self.name = 'node'
        self.K = K
        ...
        self.d_rot_as_res = d_rot_as_res
        self.opt_deform_rot = opt_deform_rot
        self.reg_loss = 0.
        self.with_node_weight = True
        self.skinning = False
        self.hyper_dim = 0  # no hyper coordinates needed

        # ARAP schedule (same as ControlNodeWarp)
        if with_arap_loss:
            self.lambda_arap_landmarks = [1e-4, 1e-4, 1e-5, 1e-5, 0]
            self.lambda_arap_steps = [0, 5000, 10000, 20000, 20001]
        else:
        ...
        # Node positions (will be initialized in init())
        self.register_buffer('inited', torch.tensor(False))
        self.nodes = nn.Parameter(torch.randn(node_num, 3))
        self._node_radius = nn.Parameter(torch.randn(node_num))
        self._node_weight = nn.Parameter(torch.zeros(node_num, 1))

        # Per-frame delta positions for control points
        self.control_point_deltas = nn.Parameter(
            torch.zeros(num_frames, node_num, 3)
        )

        # Per-frame delta rotations for control points (residual quaternions, init to zero = identity)
        if self.opt_deform_rot:
            self.control_point_rotations = nn.Parameter(
                torch.zeros(num_frames, node_num, 4)
            )

        # Frame index mapping (frame_idx -> tensor index), set during training
        self._frame_to_idx = {}
    ...
    def init(self, opt, init_pcl, force_init=False, **kwargs):
        """Initialize control nodes via FPS from point cloud."""
        if self.inited and not force_init:
            return
        self.inited.data = torch.ones_like(self.inited)
        self.register_buffer('inited', torch.tensor(True))

        if self.node_num > init_pcl.shape[0]:
            self.nodes = nn.Parameter(init_pcl.float().clone())
        else:
            init_nodes_idx = farthest_point_sample(init_pcl.detach()[None], self.node_num)[0]
            self.nodes.data = nn.Parameter(init_pcl[init_nodes_idx].float().clone())

        scene_range = init_pcl.max() - init_pcl.min()
        self._node_radius.data = torch.log(.1 * scene_range + 1e-7) * torch.ones(
            self.node_num, device=init_pcl.device)
        self._node_weight.data = torch.zeros(self.node_num, 1, device=init_pcl.device)

        # Reinit deltas for actual node count
        self.control_point_deltas = nn.Parameter(
            torch.zeros(self.num_frames, self.nodes.shape[0], 3, device=init_pcl.device)
        )
        if self.opt_deform_rot:
            self.control_point_rotations = nn.Parameter(
                torch.zeros(self.num_frames, self.nodes.shape[0], 4, device=init_pcl.device)
            )

        # Init as_gaussians
        self.init_gaussians(init_pcl=self.nodes[:, :3])
        self.as_gaussians.training_setup(opt)
        print(f'DeltaNodeWarp: initialized {self.nodes.shape[0]} control nodes from {init_pcl.shape[0]} points.')

    def init_from_base(self, opt, base_deform, force_init=True):
        """Initialize delta nodes from a trained ControlNodeWarp base model.

        Copies node positions, radii, and weights so the delta influence field
        exactly overlays the base, making small delta corrections maximally effective.
        """
        if self.inited and not force_init:
            return
        self.inited.data = torch.ones_like(self.inited)
        ...
        # Copy node positions (ControlNodeWarp nodes have hyper_dim columns, take only xyz)
        base_xyz = base_deform.nodes[:, :3].detach().clone()
        self.node_num = base_xyz.shape[0]
        self.nodes = nn.Parameter(base_xyz)

        # Copy trained radii and weights
        self._node_radius = nn.Parameter(base_deform._node_radius.detach().clone())
        self._node_weight = nn.Parameter(base_deform._node_weight.detach().clone())

        # Reinit deltas for base node count
        device = base_xyz.device
        self.control_point_deltas = nn.Parameter(
            torch.zeros(self.num_frames, self.node_num, 3, device=device)
        )
        if self.opt_deform_rot:
            self.control_point_rotations = nn.Parameter(
                torch.zeros(self.num_frames, self.node_num, 4, device=device)
            )

        # Init as_gaussians at copied node positions
        self.gs = None
        self.init_gaussians(init_pcl=self.nodes[:, :3])
        self.as_gaussians.training_setup(opt)
        print(f'DeltaNodeWarp: initialized {self.node_num} control nodes from base model.')

    def set_frame_mapping(self, frame_indices):
        """Set mapping from frame indices to tensor indices."""
        sorted_indices = sorted(frame_indices)
        self._frame_to_idx = {idx: i for i, idx in enumerate(sorted_indices)}
        self._sorted_frame_indices = sorted_indices
        self._max_frame_idx = max(frame_indices) if frame_indices else 0
    ...
    def expand_time(self, t):
        """Match ControlNodeWarp interface — expand time to (N, 1).

        Also sets _current_frame_idx from the normalized fid so that
        forward() uses the correct per-frame deltas. This avoids needing
        to set the frame index at every call site.
        """
        # Convert normalized fid [0,1] back to frame index
        if self._frame_to_idx and hasattr(self, '_max_frame_idx'):
            fid_val = t.item() if t.dim() == 0 else t.squeeze().item()
            raw_idx = round(fid_val * self._max_frame_idx)
            # Snap to nearest known frame
            if raw_idx in self._frame_to_idx:
                self._current_frame_idx = self._frame_to_idx[raw_idx]
            else:
                # Find closest known frame
                closest = min(self._sorted_frame_indices, key=lambda x: abs(x - raw_idx))
                self._current_frame_idx = self._frame_to_idx[closest]
    ...
    def node_deform(self, t, **kwargs):
        """Query deformation at control node positions for the current frame."""
        nodes = self.nodes[:, :3].detach()
        frame_idx = self._current_frame_idx
        node_deltas = self.control_point_deltas[frame_idx]
        if self.opt_deform_rot:
            rot = self.control_point_rotations[frame_idx]
        else:
            rot = torch.zeros(nodes.shape[0], 4, device=nodes.device)
        return {
            'd_xyz': node_deltas,
            'd_rotation': rot,
            'd_scaling': torch.zeros(nodes.shape[0], 3, device=nodes.device),
            'd_opacity': None,
            'd_color': None,
        }

    def trainable_parameters(self):
        # Only deltas are trainable — node positions/radii/weights are fixed after FPS init
        params = [self.control_point_deltas]
        if self.opt_deform_rot:
            params.append(self.control_point_rotations)
        return [
            {'params': params, 'name': 'deform'},
        ]

    def update(self, iteration):
        pass  # No progressive updates needed

    @torch.no_grad()
    def cal_nn_weight(self, x, K=None, feature=None, **kwargs):
        """Compute KNN weights from Gaussians to control nodes.

        Fully detached — node params are frozen for delta deform, so gradients
        should only flow through control_point_deltas, not through the weights.
        """
        if self.cached_nn_weight and self.nn_weight is not None:
            return self.nn_weight, self.nn_dist, self.nn_idxs
    ...
    def arap_loss(self, frame_idx=None):
        """Compute ARAP loss for current frame's control point positions."""
        if frame_idx is None:
            frame_idx = self._current_frame_idx

        control_pos = self.nodes[:, :3] + self.control_point_deltas[frame_idx]
        canonical_pos = self.nodes[:, :3].detach()

        ii, jj, nn, weight = cal_connectivity_from_points(canonical_pos, K=10)
        # Stack canonical and deformed as two "time steps"
        nodes_t = torch.stack([canonical_pos, control_pos], dim=0)  # (2, M, 3)
        error = cal_arap_error(nodes_t, ii, jj, nn)
        return error

    def query_network(self, x, t, **kwargs):
        """Route through forward for compatibility with ControlNodeWarp interface."""
        return self.forward(x, t, is_training=False, **kwargs)

    def densify(self, **kwargs):
        """No-op — node densification not supported for delta deform type."""
        return

    def forward(self, x, t, feature=None, motion_mask=None, iteration=0, is_training=True, **kwargs):
        """
        Compute per-Gaussian deformation via per-frame delta control points.

        Args:
            x: Gaussian positions (N, 3)
            t: time embedding (expanded, but we use _current_frame_idx instead)
            feature: ignored (no hyper-dim)
            motion_mask: (N, 1) mask

        Returns:
            dict with d_xyz, d_rotation, d_scaling, d_opacity, d_color
        """
        x = x.detach()
        frame_idx = self._current_frame_idx
        ...
        # Per-node deltas for this frame
        node_deltas = self.control_point_deltas[frame_idx]  # (M, 3)

        # Blend to get per-Gaussian translation
        translate = (node_deltas[nn_idx] * nn_weight[..., None]).sum(dim=1)
        if motion_mask is not None:
            translate = translate * motion_mask

        # Per-frame rotation deformation (residual convention: zeros = identity)
        if self.opt_deform_rot:
            node_rots = self.control_point_rotations[frame_idx]  # (M, 4)
            rotation = (node_rots[nn_idx] * nn_weight[..., None]).sum(dim=1)  # (N, 4)
            if motion_mask is not None:
                rotation = rotation * motion_mask
        else:
            rotation = torch.zeros(x.shape[0], 4, device=x.device)
        scale = torch.zeros(x.shape[0], 3, device=x.device)
        ...
        # ARAP loss
        self.reg_loss = 0.
        lambda_arap = landmark_interpolate(
            landmarks=self.lambda_arap_landmarks,
            steps=self.lambda_arap_steps,
            step=iteration,
        )
        if self.training and lambda_arap > 0 and is_training:
            arap = self.arap_loss(frame_idx)
            if not torch.isnan(arap):
                self.reg_loss = arap * lambda_arap
    ...
    def as_gaussians(self):
        if self.gs is None:
            from scene.gaussian_model import BasicPointCloud, StandardGaussianModel
            pcd = BasicPointCloud(
                points=self.nodes[:, :3].detach(),
                colors=torch.zeros_like(self.nodes[:, :3]),
                normals=self.nodes[:, :3].detach(),
            )
            self.gs = StandardGaussianModel(sh_degree=0, all_the_same=True, with_motion_mask=False)
            self.gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0., print_info=False)
            self.gs._scaling.data = torch.log(1e-2 * torch.ones_like(self.gs._scaling))
            self.gs._xyz.data = self.nodes[:, :3]
        return self.gs

    def init_gaussians(self, init_pcl, with_motion_mask=False):
        if self.gs is None:
            from scene.gaussian_model import BasicPointCloud, StandardGaussianModel
            pcd = BasicPointCloud(
                points=init_pcl.detach(),
                colors=torch.zeros_like(init_pcl),
                normals=torch.zeros_like(init_pcl),
            )
            self.gs = StandardGaussianModel(sh_degree=0, all_the_same=True, with_motion_mask=with_motion_mask)
            self.gs.create_from_pcd(pcd=pcd, spatial_lr_scale=0., print_info=False)
        return self.gs
